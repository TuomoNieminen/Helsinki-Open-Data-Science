...
test_output_contains("gni", incorrect_msg = "Please adjust the code to convert the gni variable to numeric and print out the result")
...
test_function("corrplot", args = "corr", incorrect_msg = "Please use the pipe operator to apply the `corrplot()` function on the correlation matrix of 'human_'")
...
- Access the **ggfortify** package. 
- Use autoplot to draw a biplot of `pca_human` with the 1st principal component on the x-axis and the second on the y-axis
...
*** =sample_code
```{r}
# pca_human, ggfortify, dplyr are available
...
*** =sample_code
```{r}
# pca_human, pc_lab, and ggfortify are available
