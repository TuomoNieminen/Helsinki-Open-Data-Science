...
Welcome to the *Dimensionality Reduction* chapter.
...
Most of the variable names have been shortened. See the meta file of the modified data [here](https://raw.githubusercontent.com/TuomoNieminen/Helsinki-Open-Data-Science/master/datasets/human_meta.txt). There are two new variables:

- **Edu2.FM**: The ratio of the proportions of females and males with at least secondary education (Edu2.F / Edu2.M)
- **Labo.FM**: The ratio of the proportions of females and males in the labour force. (Labo.F / Labo.M)
...
- Print out the `human` data along with an added completeness indicator. When is the indicator `F` and when is it `T`? You can also look at the help page `?complete.cases()`.
...
*** =instructions
- Use `tail()` to print out the last 10 observations of `human` (hint: `?tail`). What are the country names?
...
To study linear connections more closely, correlations can be computed with the `cor()` function and visualized with the corrplot function from the **corrplot** package.

*** =instructions
- Create the data frame `human_` by removing the `Country` variable from `human`
- Draw a scatter plot matrix of the `human_` variables with `pairs()`. Make the points more colorful by adding the argument `col = "blue"`.
...
# draw a scatter plot matrix of the 'human_' variables
...
[Principal Components Analysis](https://en.wikipedia.org/wiki/Principal_component_analysis) (PCA) can be performed by two sightly different matrix decomposition methods from linear algebra: the Eigenvalue Decomposition and the Singular Value Decomposition (SVD). 
...
A biplot is a way of visualizing the connections between two representations of the same data. First, a simple scatter plot is drawn where the observations are represented by two principal components. Then, arrows are drawn to visualize the connections between the original variables and the principal components (PCs). The following connections hold:
