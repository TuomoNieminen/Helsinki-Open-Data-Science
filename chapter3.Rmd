...
*** =sct
```{r}
test_function("plot", args = "col, pch", incorrect_msg = "Did you use the arguments `col` and `pch` in the plot fucntion?")
...
- Create a table of the results, where you compare the correct classes to the one that model produced. Did the classifier predict the crime rate correctly?
...
[Plotly](https://plot.ly/r/) is an R graphing library that creates interactive plots and has an ggplot2 integration as well. Let's try it out!
...
K-means might produce different results every time, because it randomly assigns the initial cluster centers. The function `set.seed()` 
...
In the Elbow plot the optimal number of clusters is when the within cluster sum of squares drops (there is an "elbow" in the curve). In this case,  the  Let's see how the clusters will look like. 
