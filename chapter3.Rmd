...
**obs! This exercise might have issues. If you think your correct solution isn't accepted, try refreshing the browser and submitting again.**
...
*** =instructions
- Save the scaled crime rate from the `boston_scaled` as `scaled_crim`
- Look at the summary of the scaled variable `scaled_crim`
...
*** =hint
- You can get `scaled_crim` from `boston_scaled` with the $ mark
- For labels: you can create a vector with `c()`
- For labels: Separate the values with comma in a vector
- For labels: Remember that strings need the quotes around them
...
*** =sct
```{r}
test_object("scaled_crim", incorrect_msg = "Did you create `scaled_crim` ?")
test_function("summary", args = "object", incorrect_msg = "Did you use the summary() function on the crim variable in boston_scaled data?")
test_output_contains("bins", incorrect_msg = "Did you print `bins`?")
...
*** =video_link
//player.vimeo.com/video/203184662
...
--- type:VideoExercise lang:r aspect_ratio:62.5 xp:50 skills:1 key:fc6108e633
## Video: Distance measures and clustering

*** =video_link
//player.vimeo.com/video/203198932
...
*** =sct
```{r}
test_function("kmeans", args = "centers",incorrect_msg = "Did you set the center argument in `kmeans()` ? ")
