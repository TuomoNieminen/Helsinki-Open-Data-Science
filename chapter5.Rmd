...
Continuing to ignore the repeated-measures structure of the data, we will fit a multiple linear regression model with weight as response and `Time` and `Group` (coded as two dummy variables Group2 and Group3 , with both Group2 and Group3 taking the value 0 for rats in group 1, Group2 being 1 and Group3 being 0 for rats in group 2, and Group2 being 0 and Group3 being 1 for rats in group 3) as explanatory variables.

Recall from *Chapter 1: Multiple regression* that this is done by defining explanatory variables with the `formula` argument of `lm()`, as below
...
the *random intercept model* the same two explanatory variables: `Time`
...
## Linear Mixed Effects Models: Random Intercept and Slope Model
...
to the rat growth data. Fitting a random intercept and random slope model allows the linear regression fits for each individual to differ also in slope.
