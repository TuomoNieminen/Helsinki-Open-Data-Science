...
Utilizing the pipe, we'll apply the functions `group_by()` and `summarise()` on our data. The first one splits the data to groups according to a grouping variable, and the latter can be combined with any single value summary function (such as `mean()`, `min()`, `max()`, `sd()`, ..) to summarize the data. 
