...
- Adjust the code inside the `for`-loop: if the first of the two selected columns is not numeric, simply add the first column to the `alc` data.frame.
...
- Adjust the code: Mutate `alc` by creating a new column `high_use`, which is true if `alc_use` is greater than 2 and false otherwise (replace `Inf`).
...
- Adjust the code: inside `summarise()`, after the definition of 'count', define 'mean_grade' by using `mean()` on the variable `G3`.
- Adust the code: After 'sex', add 'high_use' as another grouping variable. Execute the code again.

*** =hint
- Rememebr to separate arguments inside functions with a comma
...
- Compute the odds ratios related to the predictors by exponentiating their coefficients. Save the results to the object `OR`.
- Use `confint` on the model object `m` to extract confidence intervals for the coefficients. Then exponentiate the values with `exp()` to get confidence intervals for the odds rations. Save the results to the object `CI`.
- Combine and print out the odds ratios and the confidence intervals
...
*** =instructions
- Fit the logistig regression model with `glm()`.
...
*** =instructions
- Define the function `get_accuracy()`.
- Create object `tresholds` and print it out.
- Change the 'by' argument of `seq()` to 0.01. Create and print out `tresholds` again.
...
Cross-validation is a method of testing a predictive model on unseen data. In K-fold cross-validation, the data is split into K parts of equal size. Each part then takes turns acting as the test set, as the other parts are used to train (fit, learn) the model.
...
- Execute the call to the loss function with `prob = 1`. What kind of a prediction does this equal to? What is the interpretation of the resulting proportion?
- Execute the call to the loss function by giving it the prediction probabilities in `alc` (the column 'probability'). What is the interpretation of the resulting proportion?
- Perform leave-one-out cross-validation by using K equal to the number of observations in K-fold cross-validation. Print out the average number of wrong predictions
...
*** =sample_code
```{r}
# m and alc with predictions are available
...
*** =sct
```{r}
test_object("loss_func", incorrec_msg = "Please do not change the definition of `loss_func()`")

test_function("loss_func", args = c("class", "prob",incorrect_msg = "Please adjust the `prob` argument of `loss_func` as instructed. Do not change the `class` argument."))
test_function("cv.glm", args = c("data", "cost", "glmfit", "K"), incorrect_msg = "Please adjust the argument K of `cv.glm()` as instructed and do not adjust the other arguments.")
