...
- Adjust the code: similarily to `url_math`, make `url_por` into a valid web address using `past()` and the `url` object.
...
test_output_contains("url_por", "Please print out the object `url_por`")
...
# print out the colums not used for joining
...
for(column_name in notjoined_columns) {
  ...
  if(is.numeric(first_column)) {
    ...
    # add the fist column to the alc data frame
...
- Apply the `gather()` function on `alc` and take a glimpse at the resulting data directly after, utilizing the pipe (`%>%`). What does gather do?
...
If the 'target' function (here mean) will only use the result as an argument, the parenthesis can be dropped. 
...
From the fact that the computational target variable in the logistic regression model is the log of odds, it follows that applying the exponent function leads back to odds:
...
- Use `coef()` on the model object to extract the coefficients of the model. Then apply the `exp` function on the coefficients.
- Create object `oR` by assigning the exponentiated model coeffifients (=odds ratios) to it.
- Use `confint()` on the model object to compute (profile likelihood) confidence intervals for the coefficients. Exponentiate the values and assign the results to the object `CI`.
...
The `precict()` function can be used to make predictions with a model object. If `predict()` is not given any new data, it will use the data used for finding (fitting, leaning, training) the model to make predictions.  
...
[Cross-validation](https://en.wikipedia.org/wiki/Cross-validation_(statistics)) is a method of testing a predictive model on unseen data. In cross-validation, the value of a penalty function (mean prediction error) is computed on data not used for finding the model. 
...
*** =instructions
- Define the loss function `loss_func`
- Compute the mean prediction error for the training data: The `high_use` column in `alc` is the target and the `probability` column has the predictions.
- Perform leave-one-out cross-validation and print out the mean prediction error for the testing data. (`nrow(alc)` gives the observation count in `alc` and using `K = nrow(alc)` defines the leave-one-out method). The `cv.glm` function computes the error and stores it in `delta`. See `?cv.glm` for more information.
