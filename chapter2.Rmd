...
Logistic regression is a classification method because it can be used to classify individuals into one of two groups. Thereore it is a binary classificator.
...
- Use `confint` on the model object `m` to extract confidence intervals for the coefficients. Then exponentiate the values with `exp()`. Save the result to the object `CI`.
...
- Use `geom_point()` and draw the plot.
...
As you saw, our model can produce the predicted probabilities of the target variable 'high_use' and those probabilities can then be used to make an actual prediction. 

It makes intuitive sense to label and individual as a "high user" when the predicted probability of that is greater than 0.5. Notice that 

- using a treshold of 0 would predict everyone as a high user
- using a treshold of 1 would predict no one as a high user.

Just to convince ourselves that 0.5 is a good choice, let's try out a bunch of tresholds and see how accurately we would predict with each one. 

*** =instructions
- Define the function `get_accuracy()`.
- Create object `treshold` and print it out.
- Change the 'by' argument of `seq()` to 0.01. Create and print out `treshold` again.
- Create object `accuracy` by applying the `get_accuracy()` function on every value of `treshold`.
- Draw a plot with treshold values of the x axis and accuracy values on the y axis.
...
treshold <- seq(0, 1, by = 0.1)

# print out the tresholds


# compute the accuracy for each treshold
accuracy <- sapply(treshold, get_accuracy)

# print out the accuracies


# combine into a data frame and plot
df <- data.frame(treshold, accuracy)
ggplot(df, aes(x = "replace me", y = "and me!")) + geom_line()
...
# combine into a data frame and plot
df <- data.frame(treshold, accuracy)
ggplot(df, aes(x = treshold, y = accuracy)) + geom_line()
...
test_function("ggplot", args = "mapping", incorrect_msg = "Please use `ggplot()` to define the x and y aesthetic elements as instructed")
