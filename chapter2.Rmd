...
test_function("one_of", args = "vars", incorrect_msg = "Please use the `one_of()` function inside `select()` to create the `alc` data.frame.")
...
Mutating the data means adding new variables which are mutations of the existing ones. The `mutate()` function is from the 'dplyr' package. From now on, instead of simply loading the 'dplyr' package, we'll instead load 'tidyverse', which includes several packages that work well together, such as 'dplyr' and 'ggplot2'.

The first argument of the basic tidyverse functions is almost always 'data' and they also have other consistent features which makes them work well together.


*** =instructions
- Access the 'tidyverse' functions
...
- Mutate alc by creating logical column `high_use`, which is true if `alc_use` is greater than 2.
- Draw a bar plot of `high_use` using `ggplot()` and `geom_bar()`.
- Add (`+`) this element to the latter plot: `facet_wrap("sex")` and draw the plot again
...
# access the tidyverse functions (including dplyr and ggplot2)
library(tidyverse)

# mutate weekday and weekend alcohol use into alc_use
alc <- mutate(alc, alc_use = (Dalc + Walc) / 2)

# draw a bar plot of alc_use
ggplot(data = alc, aes(alc_use)) + geom_bar()

# mutate alc_use into a binary variable high_use
alc <- mutate(alc, high_use = 5 > 2)
...
# draw a bar plot of high_use
ggplot(data = alc, aes(high_use)) + facet_wrap("sex") + geom_bar()
...
test_function("facet_wrap", args = "facets",incorrect_msg = "")
...
I imagine you're curious to find out how the distributions of some of the other variables in the data look like. Well, why don't we draw all of them!

We'll introduce another new tidyverse toy, the pipe `%>%. The pipe operator takes the result of the left-hand side and uses it as the first argument in the function on the right-hand side. Since the first argument of the tidyverse functions is 'data' this allows for some cool chaining of commands.
...
# glimpse atlibrary("devtools")
install_github("datacamp/testwhat") the alc data
glimpse(alc) 
...
```
The pipe is most commonly used to chain functions which perform operations on a data.frame. The first argument of the tidyverse functions is almost always 'data', which makes chaining the functions with pipe easy. This is great fun, so let's try it! 

Utilizing the pipe, we'll apply the functions `group_by()` and `summarise()`. The first one splits the data to groups accoring to a grouping variable, and the latter can be combined with any single value summary function (such as `mean()`, `min()`, `max()`, `sd()`, ..) to summarize the data. 
=======
...
- Adjust the code: inside `summary()` use `mean()` on the variable `G3`, which is the (average) final grade. Execute the code again.
