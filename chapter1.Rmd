...
*** =hint
- Use the object `strategic_questions` for selecting the correct columns.
...
Often it is convenient to work with only a certain column or a subset of columns of a bigger data frame. There are many ways to select columns of a data frame in R and you are already familiar with one of them: `select()` from **dplyr***.
...
The **dplyr** library has a `rename()` function, which can also be used. Remember [the cheatseet](https://www.rstudio.com/wp-content/uploads/2015/02/data-wrangling-cheatsheet.pdf).
...
- Initialize the plotting data and aesthetic mapping
- Define the visualisation type (as points) and draw the plot
- Adjust the plot initialization: Add an aesthetic element to the plot by defining `col = gender` inside `aes()`. 
- Define the visualization type (as points) and draw the plot again
- Add a regression line to the plot
- Use `ggtitle("<insert title here>")` and give your plot the title "Student's attitude versus exam points"
- Draw the plot

*** =hint
- Use `+` to add to the plot
...
- Adjust the code: Add the argument `col` to the `pairs()` function, defining the colour with the 'gender' variable in learning2014. Draw the plot again.
- Access the **ggpot2** and **GGally** libraries and create the plot `p` with `ggpairs()`. Draw the plot.
- Adjust the code: adjust the argument `mapping` of `ggpairs()`. define `col = gender` inside `aes()`. Draw the plot again.
- Adjust the code: add another aesthetic element using `aes()` and define `alpha = 0.3`. Draw the plot again.
...
Regression analysis with R is easy once you have your data in a neat data frame. You can simply use the `lm()` function to fit a linear model. The first argument of `lm()` is `formula`, which defines the target variable and the explanatory variable(s). The formula should be

```
y ~ x
```
Where `y` is the target variable and `x` the explanatory variable. The second argument of `lm()` is `data`, which should be a data frame where `y` and `x` are columns. 

The output of `lm()` is a linear model object, which can be saved for later use. The generic function `summary()` can then be used to print out a summary of the model.
...
It is easy to include more than one explanatory variables in your linear model. The argument `formula` of `lm()` defines the explanatory variables as below

```
y ~ x1 + x2 + ..
```
where `y` is again the target variable and the `x1, x2, ..` are explanatory variables.
...
R makes it easy to graphically explore the validity of your model assumptions. If you give a linear model object as the first argument to the `plot()` function, the function automatically assumes you want diagnostic plots and will produce them. 

You can then use the argument `which` to choose which plots you want. `which` must be an integer vector corresponding to the following list of plots:

which | graphic                                 | relates to
----- | --------------------------------------- | --------
